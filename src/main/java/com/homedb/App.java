...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("Resolving users");
        UsersTable usersTable = new UsersTable(db);
        Manifest manifest = new Manifest(inputPath.resolve("manifest.json"));
        manifest.forEach(entry -> {
            if (usersTable.getId(entry.username, entry.passwordHash) == null) {
                usersTable.insertUser(entry.username, entry.passwordHash);
            }
        });

        Stream<Content> content = ContentInputReader.getContent(inputPath);
        ...
        content.forEach( c -> {
            ...
            if (ContentWriter.write(c)) {
                if (ImageContent.class.isInstance(c)) {
                    ...
                    manifest.forEach(entry -> {
                        int userId = usersTable.getId(entry.username, entry.passwordHash);
                        if (entry.contains(image.getPath())) {
                            usersTable.insertImage(userId, image.getId());
                        }
                    });
                }
                    ...
                    manifest.forEach(entry -> {
                        int userId = usersTable.getId(entry.username, entry.passwordHash);
                        if (entry.contains(video.getPath())) {
                            usersTable.insertVideo(userId, video.getId());
                        }
                    });
                }
            }
